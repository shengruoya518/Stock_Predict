...
#size:         The size of the predicted data points
def PlotData(df, size = 0):
    #Timestamp data
    ...
    originalSize = len(ts) - size
    mpl.plot(ts[0:originalSize], df.High.values[0:originalSize], color = '#727272', linewidth = 1.618, label = 'Actual')
    #Predicted data was also provided
    if(size > 0):
        mpl.plot(ts[originalSize - 1:len(ts)], df.High.values[originalSize - 1:len(ts)], color = '#7294AA', linewidth = 1.618, label = 'Predicted')
...
R = MLPR(layers, maxIter = 1000, tol = 0.10, reg = 0.001, verbose = True)
...
P = sp.PredictDate('2016-10-25', '2016-10-30', period = 'daily')
